    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        for (int j=0; j<childAry.count; j++) {
            ...
            if (_showValue) {
                ...
                label.frame = CGRectMake(bar.frame.origin.x, bar.frame.size.height*(1-grade)<20?0:bar.frame.size.height*(1-grade)-22, bar.frame.size.width, 20);
                NSArray *ary = labelArray[i];
                label.text = ary[j];
